...
unieap.define("sysSecRoleAdd", function () {

    dojo.addOnLoad(function () {

    });

    dojo.declare("_security.role.sysSecRoleAdd.View", unieap.view.View, {
        ...
        create: function () {
            ...
            this.processor = new _security.role.sysSecRoleAdd.Processor(this);
    ...
    /**
     * @description:addRole方法的成功回调。
     *
     */

    function addRoleSuccess(dc) {
        var affectRows = dc.getParameter("id");
        //插入成功后返回id值，随机产生的uuid编码
        if (affectRows.length > 2) {
            MessageBox.alert({
                title: '提示信息',
                message: '保存成功！',
                onComplete: function () {
    ...
    function form1_saveButton_onClick(event) {
        ...
        if (form.isModified()) {
            ...
            view.processor.addRole(conditionDs, 0);
        }
        else {
            MessageBox.alert({
                title: '确认框',
    ...
    var view = new _security.role.sysSecRoleAdd.View();
    view.init();
