...
function getAllHelpPage(apppath,helpMenuId){
	...
	window.open(apppath+"/techcomp/ria/allHelpInfo-view.jsp?helpMenuId="+helpMenuId +"&appName=" +unieap.appName, "allHelpFrame", sFeatures);
}

function getAllHelpPageForIconOnClick(){
	var node = unieap.getTopWin()._currentClickMenuNode;
	var menuId = null;
	if (node != null && node != undefined) {
		menuId = (node.item.data)["id"];
	}
...
function doBeforeRequest(dc,view,processorName){
	//add by liu.w
	if(unieap.getTopWin()._currentClickMenuNode){
    	dc.setParameter("FD_MENU_ID", unieap.getTopWin()._currentClickMenuNode);
    }
...
_getNavigatorContainer: function(){
				...
				if(typeof framePageContainer == 'undefined' && typeof _navigatorContainer == 'undefined'){
					dojo.require("unieapx.layout.NavigatorPane");
					dojo.require("unieapx.layout.NavigatorContainer");
					_navigatorContainer = new unieapx.layout.NavigatorContainer(
							{
								//TODO
								id : "navigatorContainer" + Math.ceil(Math.random() * 1000),
								singleAccess : true,
								style : {
									height : '100%',
									width : '100%'
					...
					_navigatorContainer.navigatorPane = new unieapx.layout.NavigatorPane( {
						title : document.title || "首页"
					})
					_navigatorContainer.addChild(_navigatorContainer.navigatorPane);
					_navigatorContainer.startup();
					setTimeout(function(){
						var body = dojo.body();
						var nodeLength = body.childNodes.length;
						var filterArray = [];
						for(var i = 0; i<nodeLength; i++){
							//弹出窗口不移动
							if(body.childNodes[i].className && ((body.childNodes[i].className.indexOf("u-xdlg")>-1) || (body.childNodes[i].className.indexOf("u-dlg")>-1))){
								filterArray.push(i);
							}
						}
						//标记要移动的node的序号
						...
						for(var i = 0; i<nodeLength; i++){
							//判断是否是弹出窗口
							if(dojo.indexOf(filterArray, i, 0, false) == -1){
								_navigatorContainer.navigatorPane.containerNode.appendChild(body.childNodes[cursor]);
							}else{
								cursor++;
							}
						}
						body.appendChild(_navigatorContainer.domNode);
						//_navigatorContainer.placeAt(dojo.body(), 'first');
					},500);
					return _navigatorContainer;
				}else if(typeof framePageContainer == 'undefined' && typeof _navigatorContainer != 'undefined'){
					return _navigatorContainer;
				}	
			...
			forward : function(scid, dcid, viewPath, title, dc, parameters){
				// add by lugj 增加对页面导航的处理
				 ...
				 navigatorContainer._afterPageLoad = function(){
					 window.setTimeout(function(){navigatorContainer.focused=true;},200);
				 };
				 ...
				 var navigatorPane = new unieap.getTopWin().unieapx.layout.NavigatorPane( {
					 ...
					 onShow : unieap.getTopWin().onSelectNavigatorPane || function(){console.info(title);},
					 height:'100%',
					 href : base
				 });
...
if (!dojo._hasResource["unieap.view.Processor"]) {
	...
	dojo.declare("unieap.view.Processor", null, {
		...
		run : function(args, _load, _error) {
		    ...
		    if (args.fileUpload)
		    {
		    	//modify by czy 2013-03-17 begin 根据入参获取参数
		    	//var formIdIndex = dojo.indexOf(paraJavatypeArr,"com.neusoft.unieap.core.common.form.Form");
		    	var formIdIndex;
//		    	for(var para in uParameters){
//		    		if("formId4Upload" == uParameters[para].name){
//		    			dc.setParameter("_methodParameterTypes", "com.neusoft.unieap.core.common.form.Form");
//		    			formIdIndex = para;
//		    		}
//		    	}
		    	var parameterTypes="";
		    	for(var kk=0;kk<uParameters.length;kk++){
		    		var para=uParameters[kk];
		    		if("formId4Upload" == para.name){
		    			parameterTypes+="com.neusoft.unieap.core.common.form.Form"
		    			formIdIndex = kk;
		    		}
		    		else
		    		{
		    			parameterTypes+=para.type;
		    		}
		    		if(kk+1<uParameters.length)
		    		{
		    			parameterTypes+=",";
		    		}
		    	}
		    	dc.setParameter("_methodParameterTypes", parameterTypes);
		    	//modify by czy 2013-03-17 end 
		    }
		    //处理高级查询参数设定
