...
//			loading.innerHTML = "正在装载缓存数据..."; // MODIFY BY TENGYF
			loading.innerHTML = RIA_I18N.patch.rpc.loadingCacheData;
...
//					loading.innerHTML="装载缓存数据失败。";
					loading.innerHTML=RIA_I18N.patch.rpc.loadCacheDataFailed;
				}
			},dc,false);	
		},
...
//	            		err.getDetail?alert(err.getDetail()):alert("获取信息失败");
	            		err.getDetail?alert(err.getDetail()):alert(RIA_I18N.patch.rpc.getInfoFailed);
	            	}            	
	            });
			}
...
//					err.getDetail ? alert(err.getDetail()) : alert("保存信息失败");
					err.getDetail ? alert(err.getDetail()) : alert(RIA_I18N.grid.rpc.saveInfoFailed);
				}
			}, dojo.toJson(value))
		},
			/**
	 ...
	 */
		doExport: function(inData){
				...
				if(!inData.layout){
//		        	 alert("导出的布局信息为空！");
		        	 alert(RIA_I18N.patch.rpc.noLayoutInfo);
		        	 return;
		        }
				var dc=new unieap.ds.DataCenter();
				var ds = inData["store"];
...
//			        	 alert("导出的布局信息为空！");
			        ...
			        if(!ds.getRowSetName()){
			        	...
			        	MessageBox.alert({
			        		type:'warn',
//			        		yesStr:'确定',
			        		yesStr:RIA_I18N.patch.rpc.confirm,
//			        		title:'提示信息',
			        		title:RIA_I18N.patch.rpc.info,
//			        		message:'由于数据资源文件配置出错,程序不执行打印操作。',
			        		message:RIA_I18N.patch.rpc.configError,
			        		motion:false
			        	})
			        	return;
			        };  
