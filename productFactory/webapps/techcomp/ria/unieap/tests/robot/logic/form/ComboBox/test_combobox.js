...
function test_attr(){
	...
	doh.register("ComboBox属性测试",[
	    
		//全选功能
		function test_selectAll(){
			var combo = unieap.byId("setSelectAll");
			var items = combo.getDataProvider().getItems();
			combo.getPopup().open();
			combo.getPopup().domNode.children[0].children[0].children[0].children[0].children[0].children[0].click();
			combo.getPopup().close();
			doh.is("5",combo.getDataProvider().getSelectedItems().length);
			combo.getPopup().open();
			combo.getPopup().domNode.children[0].children[0].children[0].children[0].children[0].children[0].click();
			combo.getPopup().close();
			doh.is("0",combo.getDataProvider().getSelectedItems().length);
		},
...
function test_method(){
	...
	doh.register("ComboBox方法测试",[
		//测试没有全选按钮时全部选中后报错
		function test_selectAll(){
			var combo=unieap.byId("setSelect");
			var items = combo.getDataProvider().getItems();
		    unieap.byId("setSelect").setSelectedItems([items[1],items[11],items[2],items[3],items[4]]);
		    combo.getPopup().open();
		    doh.t(combo.getPopup().isOpen());
		},
		//测试setValue
