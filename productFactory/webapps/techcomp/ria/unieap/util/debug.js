...
dojo.declare("unieap.util.Debug", null, {
	...
	constructor : function(json,dataCenters,pNode,showFunction,name){
		...
		var self = this;
		if(dataCenters){
			dojo.forEach(dataCenters,function(dc){
				var dcName = dc[0];
				var domNode = dc[1] && self.createJsonTree("<b>命名空间"+dcName+"的Json数据结构：</b>",dc[1]);
				if(pNode && domNode){
					pNode.appendChild(domNode);
				}	
			});
