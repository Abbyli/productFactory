...
//				_map[row.getItemValue(data.valueAttr)] = row.getItemValue(data.displayAttr);
  ...
  unieap.setPageAuthority = function(scene){
	...
	*/
	 ...
	 * 		在菜单栏打开一个新的窗口
	 * @param:
	 * 		{string} menuPath 新菜单路径
	 * @param：
	 * 		{string} id  新菜单标识
	 * @param：
	 * 		{string} menuLabel  新菜单标题
	 *
	 */
	unieap.open=function(menuPath,id,menuLabel){
	  	var topWin = unieap.getTopWin();
//	  	var menuInstance = null;
	  	...
	  	if(framePageContainer){
			...
			var menuFlag = false;
			if(!pagePane){
				pagePane = new topWin.unieap.layout.ContentPane({
					title:menuLabel,
					onClose: closeFun,
					closable : true
				})
			...
			//非单帧
			if(r != -1){
				pagePane.href = unieap.WEB_APP_NAME + menuPath;
			}
...
//				bindMenu(pagePane.controlButton.id);
				var domNode = pagePane.controlButton.id;
				if(topWin.menuInstance){
					topWin.menuInstance.bindDomNode(domNode);
				}else{
					dojo.require("unieap.menu.Menu");
					topWin.menuInstance = new topWin.unieap.menu.Menu({isSystemMenu:true});
					var item= new topWin.unieap.menu.MenuItem({label:"关闭当前",onClick:function(){
						
						var currentPane = framePageContainer.selectedChildWidget;
						var currentNode = topWin.document.getElementById(currentPane.controlButton.id);
						if(dojo.hasClass(currentNode.firstChild.lastChild,"closeImage")){
							topWin.menuInstance.unBindDomNode(currentPane.controlButton.id);																     
							framePageContainer.closeChild(currentPane);
						}
					}});
					topWin.menuInstance.addChild(item);
					item= new topWin.unieap.menu.MenuItem({label:"关闭其他",onClick:function(){
						var children = framePageContainer.getChildren();
						var currentPane = framePageContainer.selectedChildWidget;
						
						for(var i=0,child=children[0]; child=children[i];i++){
							if(child != currentPane){
								var currentNode = topWin.document.getElementById(child.controlButton.id);
								if(dojo.hasClass(currentNode.firstChild.lastChild,"closeImage")){
									topWin.menuInstance.unBindDomNode(child.controlButton.id);
	  	...
	  	function closeFun(tab,pane){
	  		if(topWin.menuInstance){
	  			topWin.menuInstance.unBindDomNode(pane.controlButton.id);
	  		}
	  		topWin.unieap.destroyDialogAndMenu(pane);
			var node = this.containerNode;
			topWin.unieap.destroyWidgets(node);		
			delete pane;
