dojo.provide('unieap.xgrid.core.foot');
dojo.require("dijit._Widget");
dojo.require("dijit._Templated");

dojo.declare('unieap.xgrid.foot', null, {
	/**
	 * @summary:
	 * 		xGrid的foot
	 * @declaredClass:
     ...
     * |			平均值：<span express="avg(attr_sal)" format="#.##" ></span>
	 ...
	 * 		format表达式支持格式转换，对形如###,#.##的格式进行转换，注意：JS会自动忽略小数点末尾无意义的数字"0"
	 * 		受限于JS的实现原理，该方法仅适合20位以内的数字的格式化
	 * 		通过grid.getFoot()得到xGrid的foot。
  	 * @img:
...
//		this.__fillContent();
    ...
    update: function(){
		if(this.footNode){
	        ...
	        dojo.forEach(expressList, function(n){
	            n.innerHTML = this._parseExpress(n.getAttribute("express"), n.getAttribute("context"), n.getAttribute("format"));
	        }, this);
		}
    },
	...
	//计算表达式
    _parseExpress: function(v, context, format){
        if (v.startWith("${")) {
            ...
            if(format){
				...
				v=dojo.number.format(v,{'pattern':format});
            }
            return v != null ? v : "&nbsp;";
        }
    },
