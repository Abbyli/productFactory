...
//		this._clearTimeOut();
	...
	onheadmousemove : function(event) {
		if (this.isHeadmove) {
			var x = event.clientX;
			var y = event.clientY;
			var moveX = x-this.startHeadx;
			var moveY = y-this.startHeady;
			...
			if ((this.winStartx + x - this.startHeadx > 0) && (moveX + w + this.winStartx< document.body.clientWidth)) {
				this._moveDiv.style.left = Math.floor(this.winStartx + x
						- this.startHeadx)
						+ "px";
			}
			if ((this.winStarty + y - this.startHeady > 0) && (moveY + h + this.winStarty< document.body.clientHeight-20)) {
