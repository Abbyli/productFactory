...
//								alert(RIA_I18N.rpc.loadError+"\n "+dojo.toJson(e,true));
				...
				try{
			            ...
			            if(_isExceptionControl()){
				            if (_dc.getCode() < 0) {
				            	...
				            	if(_dc.getParameter("SYS_ASYNC_TAG")=='99'){
				            		MessageBox.autoCloseAlert({
										title : RIA_I18N.rpc.success,
										durationTime : 2000,
									    message : "任务已经转为异步处理，稍后请上任务处理队列中查看结果。"//TODO 这里可能需要写入到国际化文件中
									});
				            	}
				            }
			            }
	 ...
	 */
	doExport: function(inData){unieap.debug(inData)
		...
		inData["unitedCells"] && dc.setParameter("unitedCells",inData["unitedCells"]);
		inData["exportType"] && dc.setParameter("exportType",inData["exportType"]);
