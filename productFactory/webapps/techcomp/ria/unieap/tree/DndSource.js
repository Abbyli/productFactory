...
dojo.declare("unieap.tree.DndSource",unieap.tree.DndSelector,{
   ...
   /**
    ...
    customDndDrop: function(source,node,target){},
	...
	_onDragMouse: function(e){
		...
		if(newTarget != oldTarget || newDropPosition != oldDropPosition){
			...
			if(!newTarget){
				...
				m.canDrop(false);
			}else if(unieap.fireEvent4Widget(this,this.widget,this.checkItemAcceptance,[newNode, m.source, newDropPosition.toLowerCase()])){
//				var targetWidget = this.tree.getNodeById(newTargetId);
//				if(targetWidget){
//					this.tree.expandNode(targetWidget);
//				}
				m.canDrop(true);
			}else{
				m.canDrop(false);
	...
	onMouseOut: function(){
   	...
   	/**
	 * @summary:
	 *      判断被拖拽（第二个参数为拖拽源）的内容是否能够置于目标节点（第一个参数）的指定位置
	...
	onDndStart: function(source, nodes, copy){
		...
		var accepted = unieap.fireEvent4Widget(this,this.widget,this.checkAcceptance,[source, nodes]);
...
//			}
			...
			var doDrop = confirm(RIA_I18N.tree.dnd.doDrop);
			this.dropConfirm = false;
			if(doDrop == true){
				var copyState = this.copyState();
				dojo.forEach(nodes, function(node, idx){
					if(source == this){
						...
						childTreeNode.item.data.id &&　this.deleteNodesId.push(childTreeNode.item.data.id);
						...
						if(source.tree){
							var nodesTemp=[];
							nodesTemp.push(node)
							newItemsParams = this.itemCreator(nodesTemp,source,targetWidget);
							//来源于其他的树，将原来树上的节点删除
							...
							this.customDndDrop(source,node,this);
						}
					}
				}, this);
				this.dropPosition != "After" && this.dropPosition != "Before" &&this.tree._expandNode(targetWidget.getDomNode());
				unieap.fireEvent4Widget(this,this.widget,this.onAfterDrop,[source, nodes]);
			}
		}
		if(nodes.length == 1 && this.anchorId){
			var anchorNode = this.tree.getNodeById(this.anchorId);
			anchorNode&&this.tree.setCurrentNode(anchorNode);
		}
		this.onDndCancel();
	},
	...
	onBeforeOverDndTree:function(source){
		return true;
	},
	
	// When focus moves in/out of the entire Tree
	onOverEvent: function(){
		if(unieap.fireEvent4Widget(this,this,this.onBeforeOverDndTree,[dojo.dnd.manager().source])){
			this.inherited(arguments);
			dojo.dnd.manager().overSource(this);
		}
	},
	onOutEvent: function(){
		if(this.dropConfirm) return;
		this._unmarkTargetAnchor();
		var m = dojo.dnd.manager();
		if(this.isDragging){
			m.canDrop(false);
