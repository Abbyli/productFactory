dojo.provide("unieap.form.InlineEditBoxValidator");
dojo.require("unieap.form.FormWidgetValidator")
dojo.declare("unieap.form.InlineEditBoxValidator",unieap.form.FormWidgetValidator,{
	/**
	 * @declaredClass:
	 * 		unieap.form.InlineEditBoxValidator
	 * @superClass：
	 ...
	 * 		支持实时校验
	 * @example:
	 * |var inlineEditBox=unieap.byId('inlineEditBox');
	 * |var validator=inlineEditBox.getValidator();
	 * |validator.validate();
	 * 		取得InlineEditbox的校验器，并进行校验。
	 * @example：
	 * |<div dojoType="unieap.form.InlineEditBox" validator="{realTime:true,regExp:/^\d+$/}"></div>
	 ...
	 */
	  ...
	  * 		{object|function}
	  * @example:
	  * |<div dojoType="unieap.form.InlineEditBox" validator="{regExp:/^\d+$/}">
	  * |</div>
	  * 	在写正则表达式时,如果已经定义了正则表达式以^符号开头以$符号结尾，请不要在斜杠后加上g，例如/^\d+$/g,这样书写会导致浏览器第一次校验成功后第二次校验必失败。
	  * @example:
	  * |<div dojoType="unieap.form.InlineEditBox" validator="{regExp:fn,exampleMsg:'不能小于100'}"></div>
	  ...
	  * 	false
	  * @example:
	  * |<div dojoType="unieap.form.InlineEditBox" validator="{regExp:/^\d+$/,realTime:true}">
	  */
	 realTime:false,
