...
dojo.declare("unieap.grid.manager.TreeManager", null, {
	...
	constructor: function(param) {
		...
		this.connects.push(dojo.connect(this.grid, "onRowSetChanged", this, "onDataChanged"));
...
//		return tree;
		...
		var row = new unieap.ds.Row(this.grid.getBinding().getRowSet(),inRowData);
		var helper = this.treeHelper;
		var tree = helper.map(row.getItemValue(this.id));
		tree = tree? helper.upgrade(row.getItemValue(this.id), inRowData)
			: helper.map(row.getItemValue(this.id), helper.createTree(inRowData));
		tree.expand = this.expandOnLoad;
		return tree;
	},
	
	link: function(tree) {
		var row = new unieap.ds.Row(this.grid.getBinding().getRowSet(),tree.rowData);
		var parentId = row.getItemValue(this.parent);
...
//		}
	...
	isLeaf: function(inTree) {
		var row = new unieap.ds.Row(this.grid.getBinding().getRowSet(),inTree.rowData);
		var parentId = row.getItemValue(this.parent);
		
		switch(row.getItemValue(this.isLeaf)) {
			case true:
			case "true":
				return true;
		}
	...
	formatNode: function(inRow, inValue) {
		var row = new unieap.ds.Row(this.grid.getBinding().getRowSet(),inRow);
	...
	findLevel: function(inId) {
		...
		if (level) {
			...
			level += this.findLevel(row.getItemValue(this.parent));
		}
		return level;
	},
	...
	//	var row = this.grid.getBinding().getRow(e.rowIndex);
		...
		var row = new unieap.ds.Row(binding.getRowSet(),binding.getRow(e.rowIndex));
		try {
...
//			}
			...
			var helper=this.treeHelper,tree = null;
			rowset.forEach(function(row){
				tree = helper.map(row.getItemValue(this.id));
				if (tree) {
					tree.expand = true;
				}
			},null,null,null,this)
