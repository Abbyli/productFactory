...
(function () {
    ...
    global.policies = {
        ...
        options: {
            ...
            containerBackgroundColor: ['backgroundColor', undefined],
    ...
    if (window.FusionMaps && window.FusionMaps.legacy) {
        global.core(['private', 'modules.core.geo',
            window.FusionMaps.legacy, window.FusionMaps.version]);
    }
    // If FusionMaps legacy was not discovered, we give it another shot after
    // the page has loaded.
    else if (!(/loaded|complete/.test(document.readyState) || document.loaded)) {
        (function () {
            ...
            window.onload = function (callback) {
                return function () {
                    init();
                    callback && callback.call && callback.call(window);
                };
            }(window.onload);
        }());
    }
...
(function () {
    ...
    var global = FusionCharts(['private', 'modules.mantle.ajax']);
            ...
            try {
                if (xmlhttp.readyState === 4) {
                    if ((!xmlhttp.status && fileProtocol) || (xmlhttp.status >= 200 &&
                        ...
                        errorCallback(Error(XHREQERROR), wrapper, data, url);
                        counters.failure++;
                    }
                    counters.idle--;
                    wrapper.open = false;
                ...
                if (errorCallback) {
                    errorCallback(error, wrapper, data, url);
                }
                counters.failure++;
            }
        };
...
(function () {
    ...
    global.normalizeCSSDimension = function (width, height, container) {
        // optimize dimensions
            var w = width === undefined ? container.offsetWidth : width,
                h = height === undefined ? container.offsetHeight : height,
                p;
    ...
    renderer = global.renderer = {
        ...
        notifyRender: function (status) {
            ...
            if (status.ref) {
                ...
                if (chartObj.options.containerBackgroundColor) {
                    status.ref.style && (status.ref.style.backgroundColor =
                        chartObj.options.containerBackgroundColor);
                }
            }
...
/*global Array: false, FusionCharts, RegExp: false, jQuery: false, window: false,
  $: true, Highcharts: false, FusionChartsDataFormats: false, FC_Loaded,
  FC_DataLoaded, FC_Rendered, FC_DrawComplete */


/**
 * This is the HighCharts Renderer bootstrap. Its basic functionalities are to
 * create interfaces between FusionCharts JS Renderer Manager and the HighCharts
 * renderer.
 *
 * @include FusionCharts.HC.js
 * @include FusionCharts.HC.Charts.js
 * @include FusionCharts.HC.PowerCharts.js
 * @include FusionCharts.HC.Widgets.js
 *
 * @since 1.0
 * @build 2450
 */
(function () {
    ...
    global = FusionCharts(['private', 'modules.renderer.highcharts']);
    // Check whether the module has been already registered. If true, then
    // do not bother to re-register.
    if (global === undefined) {
        return;
    ...
    LOADERSTYLECSSTEXT = 'display: inline-block; *zoom:1; *display:inline; width: 100%; ' +
        'font-family: Verdana; font-size: 10px; color: #666666; text-align: center;',

    MODULENAMEBASE = 'modules.renderer.highcharts-',

    // Create the renderer object.
    renderer = function () {},

    // Create an object in global that would store HighCharts Library
    lib = global.hcLib = {
        cmdQueue: []
    },
    ...
    hasModule = lib.hasModule = function (name) {
        ...
        if (name instanceof Array) {
            for (i = 0, l = name.length; i < l; i += 1) {
                if (!Boolean(global.modules[MODULENAMEBASE + name]) ||
                        (name === 'jquery' && !Boolean(window.jQuery))) {
                    return false;
                }
            }
            return true;
        ...
        return Boolean(global.modules[MODULENAMEBASE + name]);
    },
    ...
    executeWaitingCommands = lib.executeWaitingCommands = function (queue) {
        ...
        while ((item = queue.shift())) {
            ...
            // for the HighCharts renderer to get ready.
    ...
    showLoadingMessage = function (chart, container) {
        ...
        if (container && chart.options.showLoadingMessage) {
            ...
            // loaded yet.
            container.innerHTML = '<small style="'+ LOADERSTYLECSSTEXT +
                ' padding-top: ' + ((parseInt(container.style.height, 10) / 2) - 5) +'px">' +
                msgs.LoadingText + '</small>';
            // Set the container's background color if user has provided one in
            // core constructor and provided that chart is to be transparent.
            container.style.backgroundColor = vars.transparent ?
                chart.options.containerBackgroundColor || 'transparent' :
                '#ffffff';
        }
    };

    renderer.dataFormat = 'json';
    ...
    renderer.init = function () {
        /**
         * @algorithm
         * 1. Load external libraries (jQuery -> HighCharts)
         * 2. After loading is completed, render the queued charts.
         */

        // jQuery MUST be loaded in order to proceed. (Include Once is set to
        // true in order to avoid loop.)
        if (!window.jQuery) {
            loadModule('jquery', function () {
                // if jQuery was loaded by FusionCharts, we would like to set it
                // to noConflict mode and then re-assign it to $ value if no
                // other script is using $.
                jQuery.noConflict();
                if (window.$ === undefined) {
                    window.$ = jQuery;
                }
        ...
        // Now that jQuery exists, we know that 'base' has to be loaded. Had base
        // been loaded, this function would not have had existed.
        if (!hasModule('base')) {
    ...
    // This is almost a stub render function as its job is to queue objects only
    renderer.render = function (container) {
        ...
        'DataLoadRequested': function (event, args) {
            ...
            if (obj.ref && obj.options.showDataLoadingMessage) {
                if (vars.hcObj && !vars.hasNativeMessage && vars.hcObj.showLoading) {
                    vars.hcObj.showLoading(vars.msgStore.XMLLoadingText);
                }
    ...
    // Not required for interface, but needed for runtime definition of chartAPI
    renderer._call = function (fn, args, scope) {
    ...
    global.extend(global.core, {
        ...
        _fallbackJSChartWhenNoFlash: function () {
            // Check flash version.
            if (!window.swfobject.hasFlashPlayerVersion(global.core.options.requiredFlashPlayerVersion)) {
    ...
    // Check flash version.
    if (window.swfobject && window.swfobject.hasFlashPlayerVersion &&
        !window.swfobject.hasFlashPlayerVersion(global.core.options.requiredFlashPlayerVersion)) {
...
(function () {
    ...
    var json2xml = (function () {
        ...
        return function (jsonData) {
            ...
            // Call JSON2XML parser to retrieve the parsed data.
            var rootNS = jsonData && jsonData.map ? 'map' : 'chart',
...
/*properties _data, _decodePseudoCode, addDataHandler, addEventListener,
...
(function () {
    ...
    // Register the module with FusionCharts.
    var global = FusionCharts(['private', 'CSVDataHandler']);
    // Check whether the module has been already registered. If true, then
    // do not bother to re-register.
    if (global === undefined) {
        return;
    ...
    var DelimitedDataStore = function (config) {
        this._data = [];
        this.rowCount = 0;
        this.columnCount = 0;
    ...
    DelimitedDataStore.prototype.set = function (row, col, value) {
    ...
    DelimitedDataStore.prototype.configure = function (config) {
        this.delimiter = this._decodePseudoCode(config.delimiter, ',');
        this.qualifier = this._decodePseudoCode(config.qualifier, '"');
        this.eolCharacter = this._decodePseudoCode(config.eolCharacter, '\r\n');
    };

    DelimitedDataStore.prototype._decodePseudoCode = function (str, defaultValue) {
        if (str === undefined || str === null || !str.toString) {
            return defaultValue;
        }
        return str.replace('{tab}', '\t').replace('{quot}', '"')
            .replace('{apos}', '\'');
    };

    DelimitedDataStore.prototype.toString = function () {
        var row, col, str = '';

        // Iterate through all rows and generate the string structure
        for (row = 0; row < this.rowCount; row += 1) {
            col = this.qualifier + this._data[row].join(this.qualifier + this.delimiter +
     ...
     * @param {FusionCharts} obj is the FusionCharts object for which the decoding
     * will take place.
     * @param {object} options are the dataOptions passed to this data handler.
     * @type object
     */
    var decoder = function (data) { // unused param: [1]obj
        ...
        if (typeof jsondata.chart !== 'object') {
            jsondata.chart = {};
        }

        // Create a CSV based Delimyer data store
        csv = new DelimitedDataStore({
            separator: jsondata.chart.exportdataseparator,
            qualifier: jsondata.chart.exportdataqualifier
        });

        if (jsondata.dataset && jsondata.categories && jsondata.categories[0] &&
                jsondata.categories[0].category) {
            for (i = 0; i < jsondata.dataset.length; i += 1) {
                // Set the column header equalling to series name
                csv.set(0, i + 1, jsondata.dataset[i].seriesname);
                // Loop through categories and fill the values of each column
                // with its values
                for (j = 0, k = 0, l = jsondata.categories[0].category.length; j < l; j += 1) {
                    if (!jsondata.categories[0].category[j].vline) {
                        csv.set(k + 1, 0, jsondata.categories[0].category[j].label ||
                                jsondata.categories[0].category[j].name);

                        dataValue = parseFloat(jsondata.dataset[i] &&
                                    jsondata.dataset[i].data &&
                                    jsondata.dataset[i].data[k] ?
                                    jsondata.dataset[i].data[k].value : '');

                        dataValue = isNaN(dataValue) ? '' : dataValue;

                        csv.set(k + 1, i + 1, dataValue);
        ...
        } else if (jsondata.data instanceof Array) {
            csv.set(0, 1, jsondata.chart && jsondata.chart.yaxisname ?
                    jsondata.chart.yaxisname : 'Value');
            l = jsondata.data.length;
            for (i = 0; i < l; i += 1) {
                // Ignore vLines
                if (!jsondata.data[i].vline) {
                    // Set the row label.
                    csv.set(i + 1, 0, jsondata.data[i].label || jsondata.data[i].name);

                    // Set the row value.
                    dataValue = parseFloat(jsondata.data[i].value ?
                                jsondata.data[i].value : '');

                    dataValue = isNaN(dataValue) ? '' : dataValue;

                    csv.set(i + 1, 1, dataValue);
                }
            }
        }

        if (csv.rowCount > 0) {
            csv.set(0, 0, jsondata.chart && jsondata.chart.xaxisname ?
                    jsondata.chart.xaxisname : 'Label');
        }
