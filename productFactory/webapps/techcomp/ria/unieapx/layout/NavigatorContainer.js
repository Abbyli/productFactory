...
//		this.getSroll().isShowing&&this.getScroll().needScroll(page);
	...
	refresh: function(showXhrLoading){
		//debugger;
		if(this.singleAccess && this.navigatorList.length == 1) window.location.reload();
		//快速连续刷新由于页面没有渲染完可能会导致错�?
		...
		if(parentContainer){
		if((parentContainer.getParentContainer && parentContainer.getParentContainer()) || this.singleAccess){
