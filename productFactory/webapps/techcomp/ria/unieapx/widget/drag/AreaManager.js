dojo.provide("unieapx.widget.drag.AreaManager");
dojo.require("unieapx.widget.drag.Moveable");

dojo.declare(
	"unieapx.widget.drag.AreaManager",
	null,
{
	// summary:
	...
	constructor: function(){
		...
		//console.log("unieap.widget.drag.AreaManager ::: constructor");
		this._areaList = [];
	...
	init: function(){
		...
		//console.log("unieap.widget.drag.AreaManager ::: init");
	...
	registerByNode: function(/*DOMNode*/area, /*Boolean*/notInitAreas){
		...
		//console.log("unieap.widget.drag.AreaManager ::: registerByNode", area);
		...
		if(area && index == -1){
			...
			dojo.forEach(this._getChildren(area), function(item){
				...
				//this._addMoveableItem(item)返回为
				//{item:moveable对象,type:['text'],handlers:[dojo.connect句柄，监听onDragStart事件]}
				obj.items.push(this._addMoveableItem(item));
			}, this);
			//返回排序后的areaList,其obj.x坐标是从小到大。
			//areaList的为数组，其结构为:
			//dojo.mixin(obj,{x:number,y:number})
			//其中x和y分别为item的坐标
			this._areaList = this._dropMode.addArea(this._areaList, obj);
			if(!notInitAreas){
				this._dropMode.updateAreas(this._areaList);
			}
			dojo.publish("layout-drag-register",[area]);
		}
	},
	...
	registerByClass: function(){
		...
		//console.log("unieap.widget.drag.AreaManager ::: registerByClass");
	...
	unregister: function(/*DOMNode*/area){
		...
		//console.log("unieap.widget.drag.AreaManager ::: unregister");
		var index = this._getIndexArea(area);
		if(index != -1){
	...
	_addMoveableItem: function(/*DOMNode*/node){
		...
		//console.log("unieap.widget.drag.AreaManager ::: _addMoveableItem");
		node.setAttribute("tabIndex", "0");
		var handle = this._searchDragHandle(node);
		var moveable = new unieapx.widget.drag.Moveable({ 'handle': handle, 'skip': true }, node);
		// add a css style :
	...
	_deleteMoveableItem: function(/*Object*/ objItem){
		...
		//console.log("unieap.widget.drag.AreaManager ::: _deleteMoveableItem", objItem);
	...
	_getIndexArea: function(/*DOMNode*/area){
		...
		//		Get the index of an area.
	 	...
	 	// tags:
	 	//		protected
		// 获得被拖着的节点在this._areaList中的索引号

		//console.log("unieap.widget.drag.AreaManager ::: _getIndexArea");
		if(area){
	...
	_searchDragHandle: function(/*DOMNode*/node){
		...
		//console.log("unieap.widget.drag.AreaManager ::: _searchDragHandle");
		if(node){
	...
	addDragItem: function(/*DOMNode*/area, /*DOMNode*/node, /*Integer*/index, /*Boolean*/notCheckParent){
		...
		//console.log("unieap.widget.drag.AreaManager ::: addDragItem");
	...
	removeDragItem: function(/*DOMNode*/area, /*DOMNode*/node){
		...
		//console.log("unieap.widget.drag.AreaManager ::: removeDragItem");
		var index = this._getIndexArea(area);
		if(area && index !== -1){
	...
	_getChildren: function(/*DOMNode*/area){
		...
		//console.log("unieap.widget.drag.AreaManager ::: _getChildren");
		var children = [];
	...
	_setMarginArea: function(/*Object*/area,/*DOMNode*/node){
		...
		//console.log("unieap.widget.drag.AreaManager ::: _setMarginArea");
		if(area && area.margin === null && node){
			area.margin = dojo._getMarginExtents(node);
		}
	},

	findCurrentIndexArea: function(/*Object*/coords, /*Object*/size,node){
		// summary:
		...
		//		an index of area
		//coords，当前鼠标的(x,y)坐标
		//console.log("unieap.widget.drag.AreaManager ::: findCurrentIndexArea");
		//this._currenetIndexArea为当前拖着的节点所在的TD的索引
		...
		if(this._currentIndexArea != this._oldIndexArea){
			if(this._oldIndexArea != -1){
				this.onDragExit(coords, size,node);
			}
	...
	onDragStart: function(/*DOMNode*/node, /*Object*/coords, /*Object*/size){
		// summary:
		//		Initialize the drag (see unieap.widget.drag.Moveable.initOffsetDrag())
		// node:
		...
		//console.log("unieap.widget.drag.AreaManager ::: onDragStart");
		if(this.autoRefresh){
			this._dropMode.updateAreas(this._areaList);
		}
		...
		if(dojo.hasClass(node,"widgetContentSpan")){
			dojo.place(cloneNode,parent,"first");
			this.unregister(parent);
			this.registerByNode(parent)
			this._accept = false;
			this._sourceParentNode = parent;
		}else{
		...
		dojo.publish("layout-drag-start",[node, sourceArea, this._sourceDropIndex]);
	},
	...
	onDragEnter: function(/*Object*/coords, /*Object*/size){
		...
		//console.log("unieap.widget.drag.AreaManager ::: onDragEnter", coords, size);
		
		var targetArea = this._areaList[this._currentIndexArea];
		if(dojo.hasClass(targetArea.node,"titleContainer")){
			return false;
		}
	...
	onDragExit: function(/*Object*/coords, /*Object*/size,node){
		// summary:
		//		Optionally called by the getTargetArea method of TargetFinder class.
		// coords:
		//		coordinates of the dragged Node.
		...
		//console.log("unieap.widget.drag.AreaManager ::: onDragExit");
	...
	onDrag: function(/*DOMNode*/node, /*Object*/coords, /*Object*/size, /*Object*/mousePosition){
		// summary:
		//		Occurs when the unieap.widget.drag.Moveable.onDrag is fired.
		...
		//console.log("unieap.widget.drag.AreaManager ::: onDrag", node, ",", coords,size);
		var coordinates = this._dropMode.getDragPoint(coords, size, mousePosition);
		this.findCurrentIndexArea(coordinates, size,node);
	...
	placeDropIndicator: function(/*Object*/coords, /*Object*/size){
		...
		//console.log("unieap.widget.drag.AreaManager ::: placeDropIndicator");
		//keep old drop Index
	...
	onDropCancel: function(){
		...
		//console.log("unieap.widget.drag.AreaManager ::: onDropCancel");
		if(!this._accept){
	...
	onDrop: function(/*DOMNode*/node){
		...
		//console.log("unieap.widget.drag.AreaManager ::: onDrop");
		//dropCancel
		
		
		//
		
		//是从添加组件的树上拖拽下来的，此时要往目标单元格中，要添加一个widget
		if(dojo.hasClass(node,"widgetContentSpan")){
			var widgetId = dojo.attr(this._sourceParentNode,"widget"); //得到要添加的widget id
			var targetArea = this._areaList[this._currentIndexArea];
			...
			if(targetArea){
				if(targetArea.node != this._dropIndicator.node.parentNode||this._currentDropIndex==-1){
					this._currentDropIndex = targetArea.items.length;
				}
				var indexChild = this._currentDropIndex;
				dojo.publish("layout-dragdrop-addwidget",[widgetId, targetArea, indexChild]);
			}
			...
			dojo.publish("layout-drag-drop",[node, targetArea, indexChild]);
		}
	...
	destroy: function(){
		...
		//console.log("unieap.widget.drag.AreaManager ::: destroy");
		...
		if(unieapx.widget.drag.autoScroll){
			unieapx.widget.drag.autoScroll.destroy();
		}
...
unieapx.widget.drag._areaManager = null;
unieapx.widget.drag.areaManager = function(){
	// summary:
	//		Returns the current areaManager, creates one if it is not created yet.
	if(!unieapx.widget.drag._areaManager){
		unieapx.widget.drag._areaManager = new unieapx.widget.drag.AreaManager();
	}
	return unieapx.widget.drag._areaManager;	// Object
};
