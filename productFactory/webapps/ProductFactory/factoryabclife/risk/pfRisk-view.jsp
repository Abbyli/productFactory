...
<html>
	<head>
		...
		<script type="text/javascript" scope="processor" src="<%=path%>/ProductFactory/factoryabclife/risk/pfRisk-processor.js?version=20161031093232"></script>
		
		<script type="text/javascript" scope="view" src="<%=path%>/ProductFactory/factoryabclife/risk/pfRisk-view.js?version=20161031093232"></script>
		...
		<script type="text/javascript">
			if(!window["dataCenter"]){
	 		...
	 		dojo.addOnLoad(function(){
				...
				pfRisk.page_initEvents&&dojo.hitch(pfRisk,pfRisk.page_initEvents)();
				pfRisk.page_load&&dojo.hitch(pfRisk,pfRisk.page_load)();
    ...
    <div dojoType='unieap.xdialog.Dialog' height='180px' id='xdialog1' isExpand='false' title='个险定义' url='<%=path%>/ProductFactory/factoryabclife/risk_pfRiskDialog_entry.action' width='600px'>
    </div>
        ...
        <div dojoType='unieap.layout.AdaptivePane' id='adaptivePane1' width='100%'>
            <div dojoType='unieap.layout.TitlePane' flexible='false' height='100%' id='titlePane_form' title='险种查询' width='100%'>
                <div id='div_btn_form' type='buttons'>
                    ...
                    <div dojoType='unieap.form.Button' class='titlePane-button' id='btnReset' label='重&nbsp置' width='100px'></div>
                    <div dojoType='unieap.form.Button' class='titlePane-button' id='button1' label='算法测试' width='100px'></div>
                        ...
                        <tbody>
                            <tr id='form_risk_1_tr'>
                                ...
                                <td colSpan='1' id='insurtypeCode__risk_td' rowSpan='1'>
                                    <div dojoType='unieap.form.TextBox' id='insurtypeCode__risk' maxLength='6' width='100%' binding="{name:'insurtypeCode'}"></div>
                                </td>
                                ...
                                <td colSpan='1' id='verNo__risk_td' rowSpan='1'>
                                    <div dojoType='unieap.form.TextBox' id='verNo__risk' maxLength='16' width='100%' binding="{name:'verNo'}"></div>
                                </td>
                                <td>
                                </td>
                            </tr>
        ...
        <div dojoType='unieap.layout.AdaptivePane' autoHeight='true' id='adaptivePane2'>
            <div dojoType='unieap.layout.TitlePane' flexible='false' height='100%' id='titlePane_grid' title='险种列表' width='100%'>
                <div id='div_btn_grid' type='buttons'>
                    ...
                    <div dojoType='unieap.form.Button' class='titlePane-button' id='btnReview' label='提交审核' width='100px' style='display:none'></div>
                </div>
                <div dojoType='unieap.grid.Grid' height='100%' id='grid_risk' binding="{store:'tInsurtypeBasicInf_grid',rpc:pfRisk.grid_risk_binding_rpc}" rows="{rowsPerPage:'10'}" selection="{selectType:'single'}" views="{rowBar:false,rowNumber:true}">
                    <toolbar paging="{userPageSize:false}">
                    </toolbar>
                    <header>
                        <row>
                            <cell dataType='string' enable='false' id='cell_insurtypeCode__risk' label='险种编码' name='insurtypeCode' width='10%' formatter='pfRisk.cell_insurtypeCode__risk_formatter' decoder="{store:'INSURTYPE_CODE'}"></cell>
                            ...
                            <cell enable='false' id='cell_control__risk' label='操作' name='control' styles='text-align:center' width='10%' formatter='pfRisk.cell_control__risk_formatter'></cell>
                        </row>
                    </header>
                </div>
            </div>
    ...
    <div id='div1' style='position:absolute; top:0px; left:0px; background:transparent; height:99%; width:10px; z-index:99999;border:1px solid red;display:none'>
    </div>
</s:i18n>
    </security:auth>
	</body>
